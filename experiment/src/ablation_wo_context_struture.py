...
TA_OUTPUT_CSV_FILE = "ta_rag_abla_nostruct_70b_batch_faiss_flat_results.csv"
TA_OUTPUT_ACCURACY_TXT_FILE = "ta_rag_abla_nostruct_70b_batch_faiss_flat_accuracy_summary.txt"
...
def timed_llm_call_wrapper(llm_call_args: tuple) -> tuple:
    ...
    llm_response_raw = LLM_CLIENT_A.response_rag_mc(question_for_llm, context_list)
...
def evaluate_and_store_ta_rag(test_data_file: str,
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
        for item_index, item_data in enumerate(tqdm(test_data, desc="Evaluating TA-RAG performance")):
            ...
            # current_result_item["repack_time_seconds"] = repack_time
            ...
            for k_value in top_k_values_for_llm:
                ...
                # context_str_for_llm = construct_ta_rag_context(context_metadata_for_k)
                context_text_for_k = [f"{item['chunk_text']}" for item in context_metadata_for_k]
                llm_tasks_args_list.append((question_for_llm_final, context_text_for_k, k_value, item_id))
