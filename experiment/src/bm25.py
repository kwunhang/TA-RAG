...
CORPUS_JSON_FILE = "corpus_combine.json"
OUTPUT_CSV_FILE = "BM25_70b_batch_results.csv"
OUTPUT_ACCURACY_TXT_FILE = "BM25_70b_batch_accuracy_summary.txt"
...
def simple_tokenizer(text: str) -> list[str]:
    """
    A simple tokenizer that splits text by spaces and converts to lowercase.
    You might want to use a more sophisticated tokenizer for better results (e.g., NLTK, SpaCy).
    """
    return text.lower().split()

def load_corpus_and_build_bm25(corpus_json_path):
    """
    Loads corpus data from a JSON file, prepares it for BM25, and initializes the BM25 model.
    """
    if not os.path.exists(corpus_json_path):
        logging.error(f"Corpus JSON file not found: {corpus_json_path}")
        return None, None

    logging.info(f"Loading corpus from {corpus_json_path} for BM25...")
    try:
        with open(corpus_json_path, 'r') as f:
            data_iterable = json.load(f)
    except Exception as e:
        logging.error(f"Failed to load or parse corpus JSON: {e}")
        return None, None

    corpus_texts = []
    bm25_metadata_store = [] # Will store original texts and UIDs

    for i, cur_data in enumerate(tqdm(data_iterable, desc="Processing corpus for BM25")):
        corpus_uid = cur_data['corpus_uid'] # Fallback UID
        chunk_text = cur_data['chunk_text']

        if chunk_text is None:
            logging.warning(f"Missing 'chunk_text' in item {i} of corpus file. Skipping.")
            continue
        if not isinstance(chunk_text, str):
            logging.warning(f"'chunk_text' in item {i} is not a string. Skipping.")
            continue


        corpus_texts.append(chunk_text)
        bm25_metadata_store.append({
            "doc_uid": corpus_uid,
            # "chunk_number": cur_data.get('chunk_number'), # Optional, if needed later
            "chunk_text": chunk_text
        })

    if not corpus_texts:
        logging.error("No valid texts found in the corpus file.")
        return None, None

    logging.info(f"Tokenizing {len(corpus_texts)} documents for BM25...")
    tokenized_corpus = [simple_tokenizer(doc) for doc in tqdm(corpus_texts, desc="Tokenizing")]

    logging.info("Building BM25 index...")
    bm25_build_start_time = time.time()
    try:
        bm25_model = BM25Okapi(tokenized_corpus)
    except Exception as e:
        logging.error(f"Failed to initialize BM25Okapi model: {e}")
        return None, None
    bm25_build_time = time.time() - bm25_build_start_time
    logging.info(f"BM25 index built successfully in {bm25_build_time:.2f} seconds. Total documents: {len(bm25_metadata_store)}")

    return bm25_model, bm25_metadata_store

def search_bm25(bm25_model, query_text, metadata_store, top_k=10):
    if not bm25_model or not metadata_store:
        logging.warning("BM25 model or metadata_store is not loaded.")
        return [], [], 0.0, 0.0 # texts, ids, retrieval_time, query_processing_time

    tokenized_query = simple_tokenizer(query_text)

    retrieve_start_time = time.time()
    try:
        # BM25Okapi.get_scores(tokenized_query) # Gets scores for all docs
        # BM25Okapi.get_top_n(tokenized_query, corpus_texts, n=top_k) # Returns texts directly if corpus_texts is passed
        # We need indices to fetch from our metadata_store which preserves original structure and UIDs
        doc_scores = bm25_model.get_scores(tokenized_query)
        top_n_indices = np.argsort(doc_scores)[::-1][:top_k] # Get indices of top N scores

    except Exception as e:
        logging.error(f"Error during BM25 search: {e}")
        return [], [], 0.0
    ...
    for idx in top_n_indices:
        if 0 <= idx < len(metadata_store):
            ...
            logging.warning(f"Retrieved BM25 index {idx} is out of bounds for metadata_store (size {len(metadata_store)}).")

    return retrieved_texts, retrieved_ids, retrieval_time
...
def evaluate_and_store_BM25_rag(test_data_file, bm25_model, metadata_store, output_accuracy_txt_file):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS_FOR_LLM_CALLS) as executor:
        for item_index, item in enumerate(tqdm(test_data, desc="Evaluating RAG performance")):
            ...
            # BM25 here
            retrieved_chunks_texts_full, retrieved_ids_full, retrieval_time = search_bm25(
                bm25_model, org_question, metadata_store, top_k=max_k_retrieval
            )
            
            # current_result_item["embedding_time_seconds"] = embed_time
            current_result_item["retrieval_time_seconds"] = retrieval_time
            current_result_item["top_50_retrieved_ids"] = retrieved_ids_full[:max_k_retrieval]
            ...
            for k_value in top_k_values:
                context_for_current_k = retrieved_chunks_texts_full[:k_value]
            ...
            if i%20 == 0:
                ...
                for k_value_llm in top_k_values:
...
if __name__ == "__main__":
    ...
    # Load corpus and build BM25 model
    bm25_model, bm25_metadata_store = load_corpus_and_build_bm25(CORPUS_JSON_FILE)
    if not bm25_model or not bm25_metadata_store:
        logging.error("Failed to load corpus or build BM25 model. Exiting.")
        sys.exit(1)
    
    # Using dummy files for demonstration if actual paths aren't set or available
    ...
    results_df = evaluate_and_store_BM25_rag(TEST_DATA_FILE, bm25_model, bm25_metadata_store, output_accuracy_txt_path)
