...
TA_OUTPUT_CSV_FILE = "ta_rag_org_rerank_70b_batch_faiss_flat_results.csv"
TA_OUTPUT_ACCURACY_TXT_FILE = "ta_rag_org_rerank_70b_batch_faiss_flat_accuracy_summary.txt"
...
def reranking(query: str, docs: list[str], docs_metadata: list, reranker_model: FlagReranker = RERANKER) -> tuple[list[str], list[str]]:
    """
    ...
    Args:
        ...
        docs_metadata: A list of docs metadata.
    ...
    Returns:
        ...
        - reranked_docs_metadata: The list of document IDs, sorted according to the new document order.
    ...
    if reranker_model is None:
        ...
        return docs, docs_metadata # Return original order if model failed to load
    ...
    scored_docs = list(zip(scores, docs, docs_metadata))
    ...
    _, reranked_docs, reranked_docs_metadata = zip(*scored_docs)

    reranked_docs = list(reranked_docs)
    reranked_docs_metadata = list(reranked_docs_metadata)

    # logging.info(f"Reranking complete. Top score: {scored_docs[0][0]:.4f}, Bottom score: {scored_docs[-1][0]:.4f}")

    return reranked_docs, reranked_docs_metadata
...
def ta_rag_retrieval(original_query_text: str,
    ...
    # 4. Rank Candidates Semantically
    # Fallback to full search if no *query_temporal_list* was generated OR if *candidate_faiss_ids* is empty after filtering.
    # The flag `perform_full_search_if_no_temporal_candidates` controls behavior when `candidate_faiss_ids` is empty.
    # If `query_temporal_list` was empty, it implies no specific time was identified for the query.
    # If `query_temporal_list` was NOT empty, but `candidate_faiss_ids` IS empty, it means the time filter yielded no results.
    
    # Behavior decision:
    # - If query_temporal_list is present AND candidate_faiss_ids is empty: means specific time query found nothing. Don't fallback.
    # - If query_temporal_list is absent: means query was non-temporal. Fallback to full search.
    ...
    # If a temporal query was made but yielded no candidates, we should not do a full search.
    # The `rank_candidates_semantically` function's internal logic handles empty candidate_faiss_ids.
    # If `candidate_faiss_ids` is empty AND `perform_full_search` is False, it will return empty.
    # If `candidate_faiss_ids` is empty AND `perform_full_search` is True, it will do full search.
    
    # If query_temporal_list existed but filter_candidates returned empty set, then `candidate_faiss_ids` will be empty.
    # In this case, we do NOT want to `perform_full_search`.
    # So, `perform_full_search` should only be true if `query_temporal_list` itself was empty.
    
    # rerank
    max_k_retrieval = top_k * 20
    ...
    # rerank
    _, reranked_semantically_ranked_metadata = reranking(
        original_query_text,
        [item.get("chunk_text", "") for item in semantically_ranked_metadata],
    ...
    return (
        reranked_semantically_ranked_metadata[:top_k],
...
def filter_candidates_with_ncls(time_interval_pairs: list, ncls_index: NCLS) -> set:
    ...
    for q_begin_ts_float, q_end_ts_float in time_interval_pairs:        
        ...
        # For NCLS find_overlap, query range is [q_start, q_end).
        # Since our NCLS tree's ends were stored as `end_ts + epsilon`,
        # a query_end of `final_q_end_ts + epsilon` should provide consistent overlap.
        
        logging.debug(f"NCLS: Querying with floats: begin=[{final_q_begin_ts}], final_q_end_ts=[{final_q_end_ts}]")
        
        try:
            # NCLS.find_overlap returns two iterators of indices into the original arrays
            # (starts_np, ends_np, faiss_ids_np) that were used to build ncls_index.
            # We are interested in the faiss_ids_np (values) corresponding to these indices.
            # The first iterator gives indices of intervals whose START is in the query range.
            # The second gives indices of intervals whose END is in the query range.
            # For full overlap, we need intervals where interval.start < query.end and interval.end > query.start.
            # NCLS's find_overlap is designed for this.
            
            # We can iterate one of the returned iterators (e.g., iter_idx_starts)
            # and use the indices to get the Faiss IDs from the `ncls_index.values` (which is faiss_ids_np).
            
            # Note: NCLS's find_overlap requires query_start < query_end.
            # This should be ensured by `final_q_begin_ts <= final_q_end_ts` and `+1` makes end strictly larger.
